...
function TWiC_Level(p_svgParent, p_location, p_dimensions, p_levelName, p_idealText){
...
TWiC_Level.prototype.s_linkDistanceMod = 100;
TWiC_Level.prototype.s_rectCornerRadius = 15;
TWiC_Level.prototype.s_corpusMap = null;
TWiC_Level.prototype.scaleExtentLimits = [1, 16];

TWiC_Level.method("AddObjects", function(p_twicObjects, p_objectsJSON){
	this.m_twicObjects = p_twicObjects;
	this.m_objectsJSON = p_objectsJSON;
});

TWiC_Level.method("LoadLevel", function(){
	...
	this.m_svgParent.append("g")
					...
					.call(this.m_zoomBehavior.scaleExtent(TWiC_Level.prototype.scaleExtentLimits).on("zoom", this.ScrollToZoom(this)))
					.append("rect")
					.attr("class","twic_level_rect")
					.attr("id","twic_level_rect_" + this.m_levelName)
					.attr("x", this.m_location[0])
					.attr("y", this.m_location[1])
					.attr("rx", TWiC_Level.prototype.s_rectCornerRadius + "px")
					.attr("ry", TWiC_Level.prototype.s_rectCornerRadius + "px")
...
TWiC_Level.method("LoadGraph", function(){
	...
	if ( 0 == this.m_nodes.length ){
		...
		for ( var topic in TWiC_Level.prototype.s_corpusMap["topics"] ){
			if ( TWiC_Level.prototype.s_corpusMap["topics"][topic][0] < topTopicCount + 1){
				topTopics[TWiC_Level.prototype.s_corpusMap["topics"][topic][0] - 1] = TWiC_Level.prototype.s_corpusMap["topics"][topic];
			}
		}
		...
		this.m_twicObjects.push(new TWiC_Cluster(d3.select("#twic_level_" + this.m_levelName),
												[0,0], 20, topTopicCount, TWiC_Object.prototype.s_topicColors, topTopics, 
												TWiC_Level.prototype.s_corpusMap["ideal_text"], rootIndex));		
	}
	...
	this.m_graph = d3.layout.force()
							...
							.gravity(0)
							//.chargeDistance(10)
							.linkDistance(function(d){ return d.value * TWiC_Level.prototype.s_linkDistanceMod; });							
});

TWiC_Level.method("Start", function(){
...
TWiC_Level.method("ScrollToZoom", function(twic_level){
...
TWiC_Level.method("Tick", function(twicLevel){
    ...
    if ( !twicLevel.b_positionsCalculated ){
        ...
        var i = 0;
  		...
  		while (++i < n) { q.visit(collide(twicLevel.m_nodes[i])); }
